...
public:
    ...
    (
        ...
        const std::optional<std::size_t> defineSetIndex
    );
...
#ifdef HAYROLL_PIPELINE_IMPL
#error "HAYROLL_PIPELINE_IMPL should not be defined externally"
#endif

// Implementation in header by design (not inline). Ensure only included where needed.
namespace Hayroll
{

using json = nlohmann::json;

std::filesystem::path Pipeline::saveOutput
(
    const Hayroll::CompileCommand & base,
    const std::filesystem::path & outputDir,
    const std::filesystem::path & projDir,
            ...
            try
            {
                ...
                std::vector<DefineSet> defineSets = Splitter::run(premiseTree, command);
                {
                    ...
                    for (std::size_t i = 0; i < defineSets.size(); ++i)
                    {
                        ...
                        std::filesystem::path seededRsPath = Pipeline::saveOutput
                        (
                            command,
                            outputDir,
                            projDir,
