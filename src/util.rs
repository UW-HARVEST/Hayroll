...
use syntax::{
    ast::{self, edit_in_place::AttrsOwnerEdit, HasAttrs, SourceFile},
...
pub fn parent_until_kind_and_cond<T>(
        ...
        .filter(|item| {
            item.attrs().any(|attr| {
                attr.meta().map_or(false, |meta| {
                    meta.path()
                        .map_or(false, |path| path.to_string() == "c2rust::src_loc")
                        && meta.expr().map_or(false, |expr| {
                            ast::String::cast(expr.syntax().first_token().unwrap()).map_or(
                                false,
                                |string| {
...
impl SourceChangeBuilderSet {
    ...
    pub fn get(&mut self, file_id: FileId) -> &mut SourceChangeBuilder {
        self.builders
            .get_mut(&file_id)
            .expect("No builder for file id (did you forget from_syntax_roots?)")
    }

    pub fn builder_mut(&mut self, file_id: FileId) -> &mut SourceChangeBuilder {
        self.get(file_id)
    }
    ...
    pub fn builder_mut_for_node(&mut self, node: &syntax::SyntaxNode) -> &mut SourceChangeBuilder {
        let file_id = self
            .file_id_of_node(node)
            .expect("Unable to resolve FileId from node root");
        self.get(file_id)
    }
    ...
    pub fn make_mut<N: syntax::AstNode>(&mut self, node: N) -> N {
        let file_id = self
            .file_id_of_node(node.syntax())
            .expect("Unable to resolve FileId from node");
    ...
    pub fn finish(mut self) -> ide::SourceChange {
        ...
        self.builders
            .into_iter()
            .fold(ide::SourceChange::default(), |acc, (_fid, builder)| {
